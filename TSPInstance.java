...
public class TSPInstance extends Instance {
	...
	public TSPInstance (int n) {
		...
		for (State s : states) {
			((TSPState) s).initProbabilities();
			((TSPState) s).initDistances();

		}
	}

	public TSPInstance (File f) {
		...
		try {
			...
			double scale = (SIZE-20) / size;
			double xextra_room = (1 - xsize / size) * SIZE / 2;
			double yextra_room = (1 - ysize / size) * SIZE / 2;
